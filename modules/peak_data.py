...
class GridImageShelve:
    """
    A class to generate, store, and retrieve grid images for given peak and sample.
    ...
    def create_grid_image(self, maindata, peak, sample):
        """
        Given the main DataFrame, a peak, and a sample,
        this method generates the grid image.
        
        Parameters:
            maindata (DataFrame): The complete data.
            peak (str): The peak column to process.
        ...
        grid = create_section_grid(maindata, section_ids, peak, cols=cols)
    ...
    def store_grid_image(self, peak, sample, grid_image):
        """
        Stores the grid image in the shelve database with a key based on peak and sample.
        
        Parameters:
            peak (str): The peak identifier.
            sample (str): The sample identifier.
            grid_image (np.ndarray): The grid image to store.
        """
        key = f"{peak}_{sample}_grid"
        with shelve.open(self.shelf_path) as db:
            db[key] = grid_image

    def retrieve_grid_image(self, peak, sample=None, slice_index=None):
        """
        Retrieves the grid image for the given peak and sample from the shelve database.
        ...
        with shelve.open(self.shelf_path) as db:
            if key in db:
                ...
                raise KeyError(f"Grid image for peak '{peak}' and sample '{sample}' not found.")

    def process_maindata(self, maindata, peaks=None, samples=None):
        """
        Processes the main DataFrame by generating and storing grid images for each
        combination of peak and sample.
        
        Parameters:
            maindata (DataFrame): The complete data.
            peaks (iterable, optional): List of peaks to process. If None, uses the first 173 columns.
        ...
        if peaks is None:
            peaks = maindata.columns[:173].values
        if samples is None:
            samples = maindata['Sample'].unique()

        for peak in tqdm(peaks):
            for sample in samples:
                grid_image = self.create_grid_image(maindata, peak, sample)
                self.store_grid_image(peak, sample, grid_image)
                print(f"Stored grid image for peak '{peak}' and sample '{sample}'.")